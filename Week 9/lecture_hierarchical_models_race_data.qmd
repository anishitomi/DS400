...
```{r}
ggplot(data = running, aes(x = age, fill = runner)) +
  geom_bar() +
  coord_flip()
```
...
```{r}
# Remove rows with missing or invalid times
cherry_data_clean <- running[!is.na(running$net) & running$net > 0, ]

# Create the box plot with clean data
ggplot(cherry_data_clean, aes(x = runner, y = net)) + 
  geom_boxplot(fill = "lightblue", color = "darkblue") +
  labs(title = "Box Plot of Runner Times (Cherry Blossom Run)", 
       y = "Time (minutes)") +
  theme_minimal()
...
```{r}
ggplot(data = running, aes(x = as.factor(age), y = net)) +
  geom_boxplot() +
