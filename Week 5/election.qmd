...
```{r}
plot_beta_binomial(alpha = 45, beta = 55, y = 60, n = 100)
```
...
```{r}
plot_beta_binomial(alpha = 95, beta = 5, y = 8, n = 39)
```

**Exercise 3.17 (Plotting the Beta-Binomial: Take II)** Repeat Exercise [3.16](https://www.bayesrulesbook.com/chapter-3#exr:ch3-bbplot) for the `plot_beta_binomial()` output below.

```{r}
plot_beta_binomial(alpha = 50, beta = 50, y = 8, n = 39)
```
