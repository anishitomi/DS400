...
ggplot(data = fake_news, aes(x = type, fill = title_has_excl)) + 
  geom_bar() +
  labs(title = "Title has excl", x = "Type of Article", y = "Count")
...
```{r}
fake_news %>% 
  tabyl(type) %>% # gets the number of types in a particular column
  adorn_totals("row") # adds totals by row
...
```{r}
likelihood_article_fake_has_excl <- .27
```
...
```{r}
# Prior Probabillity person is from the South region
pop_vs_soda %>% 
  tabyl(region) %>% # gets the number of types in a particular column
  adorn_totals("row") # adds totals by row
```

```{r}
prior_probability_someone_from_south <- .24
```

```{r}
# Prior probabillity someone says pop
pop_vs_soda %>% 
  tabyl(pop) %>% 
  adorn_totals("row")
```
...
```{r}
prior_probability_someone_says_pop <- .37
```

```{r}
# Liklihood someone from south says pop
pop_vs_soda %>% 
  tabyl(pop, region) %>% 
  adorn_percentages("col") 
```
...
```{r}
likelihood_somone_from_south_says_pop <- 0.07922231
```

```{r}
(prior_probability_someone_from_south * likelihood_somone_from_south_says_pop) / prior_probability_someone_says_pop
```
...
Census Data of where people are from

```{r}
prior <- c(midwest = 0.21,
           northeast = 0.17,
           south = 0.38,
           west = 0.24)
...
```{r}
prior_probabillity_says_pop <- sum(prior * likelihoods)

prior_probabillity_says_pop
```

4\) Finding our posterior probability P(S\|A)

```{r}
(prior[3] * likelihoods[3]) / prior_probabillity_says_pop
```
